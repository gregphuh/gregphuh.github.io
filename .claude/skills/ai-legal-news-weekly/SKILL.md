...
description: Writes the WEEKLY "AI Law Weekly" articles only — the second of two AI Law Weekly skills. Use when the user or a Claude Routine asks for the AI Law Weekly weekly writeup, weekly AI article, hot topics, or top stories of the week. Runs unattended and non-interactively — reads the most recent daily-log snapshot on Google Drive, ranks the week's entries by a hotness rubric, automatically selects the top 2 (different practice areas where possible), and drafts them as .doc Word articles of 600–650 words each (built from a fixed HTML template — Georgia font, black headings, round-bullet Takeaways, a numbered Endnotes section, blue hyperlinked citations, fixed "AI Law Weekly · [Date]" byline) saved to the articles folder — never asking which to write. Do NOT use for the daily log capture (use the ai-legal-news-log skill) or for non-US / non-legal AI news.
---
...
and turns the week's top stories into finished Word articles (`.doc` files built from the fixed HTML template embedded in `references/article-template.md`).
...
**6. Adversarial review (mandatory — a second, skeptical pass).** Before anything is saved, switch into a separate **Reviewer** role and review each draft adversarially: assume it is wrong until proven otherwise; do not rubber-stamp. If the runtime supports subagents, run this as a separate agent for independence. The Reviewer must, at minimum:
   ...
   - **Enforce limits:** body **600–650 words — reject and fix if under 600 or over 650** (a short draft that merely resembles an example is not acceptable); ≤3 endnotes; copyright discipline (<15 verbatim words per source, ≤1 quote per source).
   - **Format integrity:** the output must be the embedded HTML template (the fenced block in `references/article-template.md`) with **only the text changed** — the `<head>`/`<style>` block byte-for-byte identical, all template classes intact, no tag/style altered, byline exactly `AI Law Weekly · [Date]`, and the Background/Analysis/Takeaways headings + Endnotes list all present. Reject any draft whose formatting differs from the template.
...
> Build each by copying the embedded HTML template in `references/article-template.md` verbatim and replacing only the text content —
