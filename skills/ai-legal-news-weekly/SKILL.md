...
and turns the week's top stories into finished Word articles (`.doc` files built from the fixed HTML template `references/article-template.txt`).
...
> Build each by copying `references/article-template.txt` verbatim and replacing only the text content —
